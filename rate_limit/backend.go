...
type Backend interface {
	...
	BudgetAvailable(provider groq.Provider) (tokensAvailable int, requestsAvailable int)

	// RecordConsumption records token and request usage for the given provider.
	RecordConsumption(provider groq.Provider, tokens int, requests int) error
	...
	SetBudgetForTests(provider groq.Provider, tokens int, requests int) error
